...
struct LiquidSwipeView: View{
    ...
    private var showSlider: CGFloat{ CGFloat(Show ? alpha : 0.0) }
    
    var body: some View {
        ZStack {
            content().opacity(alpha)
            slider(data: $leftData).opacity(Double(showSlider))
            slider(data: $rightData).opacity(Double(showSlider))
    ...
    func button(data: SliderData) -> some View {
        ...
        .overlay(Text("sgssf"), alignment: .bottom)
