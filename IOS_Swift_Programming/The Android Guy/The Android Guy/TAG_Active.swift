...
struct TAG_Active: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        var lhand = Path()
        lhand.move(to: CGPoint(x: center.x - radius * 0.8, y: center.y - radius * 0.7))
        lhand.addLine(to: CGPoint(x: center.x - radius * 0.8, y: center.y - radius * 0.15))
        lhand.addQuadCurve(to: CGPoint(x: center.x - radius * 0.55, y: center.y - radius * 0.15),
                           control: CGPoint(x: center.x - radius * 0.675, y: center.y))
        lhand.addLine(to: CGPoint(x: center.x - radius * 0.55, y: center.y - radius * 0.7))
        lhand.addQuadCurve(to: CGPoint(x: center.x - radius * 0.8, y: center.y - radius * 0.7),
                           control: CGPoint(x: center.x - radius * 0.675, y: center.y - radius * 0.85))
        
        var rhand = Path()
        rhand.move(to: CGPoint(x: center.x + radius * 0.55, y: center.y - radius * 0.7))
        rhand.addLine(to: CGPoint(x: center.x + radius * 0.55, y: center.y - radius * 0.15))
        rhand.addQuadCurve(to: CGPoint(x: center.x + radius * 0.8, y: center.y - radius * 0.15),
                           control: CGPoint(x: center.x + radius * 0.675, y: center.y))
        rhand.addLine(to: CGPoint(x: center.x + radius * 0.8, y: center.y - radius * 0.7))
        rhand.addQuadCurve(to: CGPoint(x: center.x + radius * 0.55, y: center.y - radius * 0.7),
                           control: CGPoint(x: center.x + radius * 0.675, y: center.y - radius * 0.85))
        ...
        rightantennae.move(to: CGPoint(x: center.x + radius * 0.25, y: center.y - radius * 0.781))
        ...
        rightantennae.addQuadCurve(to: CGPoint(x: center.x + radius * 0.32, y: center.y - radius * 0.96),
                                   control: CGPoint(x: center.x + radius * 0.34, y: center.y - radius * 0.98))
        rightantennae.addLine(to: CGPoint(x: center.x + radius * 0.22, y: center.y - radius * 0.8))
